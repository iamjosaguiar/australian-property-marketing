...
export async function generateMetadata({
  ...
  return {
    title: `Floor Plans ${state.name} | 2D & 3D Property Floor Plans ${state.code} | Australian Property Marketing`,
    description: `Professional floor plans across ${state.name}. 2D and 3D plans from $95 with 24-hour delivery. Accurate measurements. Book online today.`,
    openGraph: {
      title: `Floor Plans ${state.name} | Australian Property Marketing`,
      description: `Professional 2D and 3D floor plans in ${state.name}. From $95. 24-hour delivery.`,
      type: 'website',
      locale: 'en_AU',
    },
  };
...
export default async function StateFloorPlansPage({
  params,
}: {
  params: Promise<{ state: string }>;
}) {
  ...
  try {
    ...
    suburbs = stateSlug === 'nsw' ? [
      { id: 1, name: 'Bondi', slug: 'bondi', stateSlug: 'nsw', city: 'Sydney', floorPlanPrice: 95, priority: 10 },
      { id: 2, name: 'Paddington', slug: 'paddington', stateSlug: 'nsw', city: 'Sydney', floorPlanPrice: 95, priority: 9 },
      { id: 3, name: 'Surry Hills', slug: 'surry-hills', stateSlug: 'nsw', city: 'Sydney', floorPlanPrice: 95, priority: 10 },
    ] : [];
  }
        ...
        <section className="relative h-[500px] bg-gradient-to-br from-navy-900 to-slate-800">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6 h-full flex flex-col justify-center">
            <h1 className="text-5xl md:text-6xl font-black text-white mb-4">
              Floor Plans in {state.name}
            </h1>
            <p className="text-xl text-white/90 mb-8 max-w-2xl">
              Professional 2D and 3D floor plans across {state.name}. From $95 with 24-hour delivery. Serving {suburbs.length} locations statewide.
            </p>
